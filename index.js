...
app.get('/hotels/sort/pricing', (req, res) => {
  let sortType = req.query.pricing.toLowerCase();
  let hotelsCopy = hotels.slice();
  if (sortType == 'low-to-high') {
    hotelsCopy.sort(sortByPricingLowToHigh);
  } else {
    hotelsCopy.sort(sortByPricingHighToLow);
  }
...
app.get('/hotels/sort/rating', (req, res) => {
  let sortType = req.query.rating.toLowerCase();
  let hotelsCopy = hotels.slice();
  if (sortType == 'low-to-high') {
...
app.get('/hotels/sort/reviews', (req, res) => {
  let sortType = req.query.reviews.toLowerCase();
  let hotelsCopy = hotels.slice();
  if (sortType == 'least-to-most') {
    hotelsCopy.sort(sortByReviewLowToHigh);
  } else if (sortType == 'most-to-least') {
    hotelsCopy.sort(sortByReviewHighToLow);
  }
  res.json({ hotels: hotelsCopy });
});
...
function filterHotelByAmenities(hotel, amenity) {
...
app.get('/hotels/filter/amenity', (req, res) => {
  ...
  let filteredHotels = hotels.filter((hotel) =>
    filterHotelByAmenities(hotel, amenity)
  );
  res.json({ hotels: filteredHotels });
});

//5 Based on country
function filterHotelByCountry(hotel, country) {
...
app.get('/hotels/filter/country', (req, res) => {
  ...
  let filteredHotels = hotels.filter((hotel) =>
    filterHotelByCountry(hotel, country)
  );
  res.json({ hotels: filteredHotels });
});

//6 Based on category
function filterHotelByCategory(hotel, category) {
...
app.get('/hotels/filter/category', (req, res) => {
  ...
  let filteredHotels = hotels.filter((hotel) =>
    filterHotelByCategory(hotel, category)
  );
  res.json({ hotels: filteredHotels });
});
